...
export class DashboardLibraryBooks extends Component {
  ...
  indexCount = () => {
    return Math.ceil(
      this.libraryBooks().length / this.cardCount()
    );
  };
  ...
  libraryBooks = () => {
    const reservedBooks = this.props.reservedBooks.filter(r =>
      r.delivered === true
    );

    return this.props.books.filter(book =>
      book[1].id !== this.props.auth.id
      && !reservedBooks.find(r => r.user_lib_book_id === book[2])
    );
  };
          ...
          <Grid>
            <Grid.Row>
              ...
              <Grid.Column width='14'>
                ...
                <div className={this.props.userDash 
                  ? 'ui five centered cards' 
                  : 'ui eight centered cards'
                }>
                  {this.libraryBooks().slice(
                    this.currentStartIndex(),
                    this.currentEndIndex()
                  ).map((book, i) => {
                    if (this.wishedBooks().find(b =>
